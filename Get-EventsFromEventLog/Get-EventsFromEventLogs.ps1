...
.EXAMPLE 
.\Get-EventsFromEventLogs.ps1 -EventSourceToCheck "Outlook"
...
----------- -------     -----------           ---------------- -- -------
akasammy-PC Application 4/13/2018 11:57:06 AM Information      63 La demande de service web Exchange GetAppManifestssuccède à.</0w>
akasammy-PC Application 4/13/2018 7:57:00 AM  Information      63 La demande de service web Exchange GetAppManifestssuccède à.</0w>
akasammy-PC Application 4/13/2018 7:56:59 AM  Information      63 Outlook a détecté une notification de modification pour vos applications et va t...
akasammy-PC Application 4/13/2018 7:56:55 AM  Information      45 Outlook a chargé le(s) complément(s) suivant(s) :...
...
Param(
    ...
    [Parameter(Mandatory = $False, Position = 4)] $EventSourceToCheck="None",
    [Parameter(Mandatory = $False, Position = 5)] [int]$NumberOfLastEventsToGet = 30,
    [Parameter(Mandatory = $False, Position = 6)] [Switch]$ExportToFile
)
...
while ($Answer -ne "Y" -AND $Answer -ne "N") {
    ...
    Write-Host "Event Source to check   :   $EventSourceToCheck"
...
If ($EventSourceToCheck -eq "None") {
    ...
    If ($EventIDToCheck -eq "None" -or $EventIDToCheck -eq "" -or $EventIDToCheck -eq $Null -or $EventIDToCheck -eq 0){
        $FilterHashProperties = @{
            LogName = $EventLogName;
            ProviderName = $EventSourceToCheck;
        }
    } else {
            $FilterHashProperties = @{
                LogName = $EventLogName;
